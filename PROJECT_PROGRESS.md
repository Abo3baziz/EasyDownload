...
- Child processes (yt-dlp, FFmpeg) are spawned with `PYTHONUTF8=1` / `PYTHONIOENCODING=utf-8` so Python-based yt-dlp emits UTF-8 output regardless of the Windows ANSI code page; streamed output is decoded with a `StringDecoder` to avoid corrupting multi-byte characters split across chunks. This keeps non-ASCII (e.g. Arabic) titles and destination paths intact.
